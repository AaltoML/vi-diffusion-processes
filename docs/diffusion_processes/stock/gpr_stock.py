...
def optimize_gpr(model: GPR, lr: float, time_grid: tf.Tensor, test_obs: [tf.Tensor, tf.Tensor],
    ...
    loss_vals = [model.training_loss()]
    nlpd_vals = [get_model_nlpd(model, time_grid, test_obs)]
    if wandb.run:
        wandb.log({"Loss": loss_vals[-1], "nlpd": nlpd_vals[-1]})

    for i in range(max_itr):
        opt_step()
        loss_vals.append(model.training_loss())
        nlpd_vals.append(get_model_nlpd(model, time_grid, test_obs))
        logger.info(f"Loss: {loss_vals[-1]}")
...
@hydra.main(version_base="1.2", config_path="../configs/", config_name="gpr_stock_process")
def run_experiment(cfg: DictConfig):
    ...
    model = instantiate(cfg.model)(data=observations, kernel=kernel, noise_variance=noise_stddev[0][0] ** 2)
    ...
    if cfg.optimize:
        optimize_gpr(model, lr=cfg.lr, optim_tol=cfg.optim_tol, max_itr=cfg.max_itr, time_grid=time_grid,
                     test_obs=test_obs)
    model: GPR
    logger.info(f"Loss : {model.training_loss()}")
    ...
    if wandb.run:
        if not cfg.optimize:
            wandb.log({"Loss": model.training_loss(), "nlpd": nlpd_val, "rmse": rmse_val})
    ...
    np.savez(os.path.join(output_dir, "training_statistics.npz"), loss=model.training_loss(), nlpd=nlpd_val,
             rmse=rmse_val)
    tf.saved_model.save(model, os.path.join(output_dir, "gpr_model"))
